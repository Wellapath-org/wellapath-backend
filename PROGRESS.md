...
**Staging note (2026-07-29):** `GET /health` is returning 503 `database: error` — the Supabase pooler reports the tenant does not exist. `/config` and `/version` are unaffected (neither touches the database; the only runtime query in the app is the health check's `SELECT 1`), so the mobile bootstrap path is intact. Escalated to the engineering lead — infrastructure action, not a code defect.\_
